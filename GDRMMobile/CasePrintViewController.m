...
-(NSString *)xmlStringFromFile:(NSString *)xmlName{
   ...
   // xmlString=[NSString stringWithContentsOfFile:[NSString stringWithFormat:@"/Users/admin/项目/GDRMMobileXBYH/GDRMMobile/XML/%@.xml",xmlName] encoding:NSUTF8StringEncoding error:nil];
    return xmlString;
}
    ...
    if (staticTables != nil) {
                ...
                if (textInXML != nil) {
                    ...
                    if (![textString isEmpty]) {
                        ...
                        if (originInXML != nil) {
                            ...
                            CGFloat x=[[TBXML valueOfAttributeNamed:@"x" forElement:originInXML] floatValue];
                            CGFloat y=[[TBXML valueOfAttributeNamed:@"y" forElement:originInXML] floatValue];
        ...
        if (texts != nil) {
            ...
            while (text) {
                ...
                if (textInXML != nil) {
                    NSString *textString=[TBXML textForElement:textInXML];              //案号
                    if (![textString isEmpty]) {
                        ...
                        if (originInXML != nil) {
                            ...
                            if (alignmentInXML) {
                                ...
                                NSLog(@"-------测试------alignmentinxml%@",alignmentString);
...
- (void)drawDateTable:(NSString *)xmlName withDataModel:(NSManagedObject *)data{
    ...
    if (data) {
        ...
        TBXMLElement * root = tbxml.rootXMLElement;
        TBXMLElement *paperSetting=[TBXML childElementNamed:@"Paper" parentElement:root];
...
- (NSString *)formedStringFromData:(NSManagedObject *)data XMLElement:(TBXMLElement *)xmlElement{
    ...
    TBXMLElement *formatorNameInXML=[TBXML childElementNamed:@"formatorName" parentElement:xmlElement];
    NSString *attributeName=@"";
    NSString *entityName=@"";
    NSString *formatorName=@"";
    if (attriNameInXML) {
    ...
    if (formatorNameInXML) {
        formatorName=[TBXML textForElement:formatorNameInXML];
    }
    ...
    if (![attributeName isEmpty]) {
        ...
        NSLog(@"________dataobj所传数据：%@",dataObj);
        if (dataObj) {
            if ([dataObj respondsToSelector:NSSelectorFromString(attributeName)]) {
                ...
                valueObj=objc_msgSend(dataObj, NSSelectorFromString(attributeName));
                NSLog(@"！！测试！！！！！！！！！%@下个%@下个%@",attributes,attributeName,valueObj);
                if (valueObj){
                    switch (attriDesc.attributeType) {
                        case NSStringAttributeType:
                            tempString = [tempString stringByAppendingString:valueObj];
                            NSLog(@"1测试tempString：%@",tempString);
                            break;
                        case NSFloatAttributeType:
                        case NSDoubleAttributeType:{
                            if([formatorName isEqualToString:@"#.##"]){
                                 tempString=[tempString stringByAppendingString:[valueObj stringValue]];
                                NSLog(@"2测试tempString：%@",tempString);
                            }
                            else{
                            ...
                            NSLog(@"3测试tempString：%@",tempString);
                        }
                            break;
