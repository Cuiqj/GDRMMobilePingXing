...
//        [RoadSegment roadNameFromSegment:self.caseInfo.roadsegment_id];
    ...
    [self.citizenInfoBriefVC loadCitizenInfoForCase:caseID andNexus:@"当事人"];   //添加当事人
}
...
//    [NSString stringWithFormat:@"%d", [self.roadSegmentID intValue]];
...
//    //保存询问笔录数据
//    [self saveInquireForCase:caseID];
    //案由是保存在勘验检查笔录中的
...
- (void)saveCaseProveInfoForCase:(NSString *)caseID{
    ...
    NSString * strKM = @"";
    if([self.caseInfo.station_start integerValue] != 0){
        strKM = [NSString stringWithFormat:@"k%ld+%ldm",[self.caseInfo.station_start_km integerValue],[self.caseInfo.station_start_m integerValue]];
    }
    
    caseProveInfo.remark = [NSString stringWithFormat:@"%@%@%@%@",self.textRoadSegment.text,self.textSide.text,strKM,self.textPlace.text];
