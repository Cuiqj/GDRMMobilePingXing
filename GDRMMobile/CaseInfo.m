...
//读取下载数据
+(NSArray *)caseInfoForDownDatawithtypeID:(NSString *)typeID{
...
//    NSSortDescriptor *sortByCaseMark3=[NSSortDescriptor sortDescriptorWithKey:@"case_mark3.integerValue" ascending:NO];
    NSSortDescriptor *sortByCaseMark2=[NSSortDescriptor sortDescriptorWithKey:@"happen_date" ascending:NO];
    NSArray *sortArray=[NSArray arrayWithObjects:sortByCaseMark2,nil];
    [fetchRequest setSortDescriptors:sortArray];
    [fetchRequest setEntity:entity];
    [fetchRequest setPredicate:[NSPredicate predicateWithFormat:@"case_mark2 != nil && case_mark3 != nil && case_type_id == %@ && isdowndata == YES",typeID]];
    NSError *error;
...
- (NSString *)station_start_km{
    if([self.station_start integerValue]==0)
        return @"";
...
- (NSString *)full_happen_place{
    ...
    else if ([self.station_end integerValue] ==0 && [self.station_start integerValue] == 0){
        return [NSString stringWithFormat:@"%@%@%@", roadName, self.side,self.place];
    }
    else
