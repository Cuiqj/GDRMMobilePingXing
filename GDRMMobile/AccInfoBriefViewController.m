...
- (void)viewDidLoad
{
    ...
    self.textreason.tag = 555;
    self.textCaseStyle.tag = 556;
    self.textCaseType.tag = 557;
...
-(void)saveDataForCase:(NSString *)caseID{
    ...
    if (caseInfo){
        caseInfo.case_reason    = _textreason.text;                         //安全距离不足  事故原因
        ...
        caseInfo.case_type      = _textCaseType.text;                   //碰撞护栏及交通设施    事故类型
        caseInfo.case_style     = _textCaseStyle.text;                      //轻微事故   事故性质
        ...
        //
        //    caseInfo.place =  超车道 右侧
    }
    NSArray* times=[Times TimesByParent_id:caseID];
    NSDateFormatter *dataFormater=[[NSDateFormatter alloc]init];
    [dataFormater setLocale:[NSLocale currentLocale]];
    dataFormater.dateFormat=@"yyyy-MM-dd HH:mm";
    if(times.count>0){
        for (Times *atime in times) {
            NSString* name =[atime valueForKey:@"name"];
            NSString *timestr=[[self valueForKey:name]  valueForKey:@"text"];
            NSDate *date=[dataFormater dateFromString: timestr];
            [atime setValue:date forKey:@"time"];
            [[AppDelegate App]saveContext];
        }
    }else{
        Times *atime=[Times newDataObjectWithEntityName:@"Times"];
        [atime setValue:@"jiebaoTime" forKey:@"name"];
        [atime setValue:caseID forKey:@"parent_id"];
        [atime setValue:[dataFormater dateFromString:self.jiebaoTime.text] forKey:@"time"];
        
        [[AppDelegate App]saveContext];
        atime=[Times newDataObjectWithEntityName:@"Times"];
        [atime setValue:@"daochangTime" forKey:@"name"];
        [atime setValue:caseID forKey:@"parent_id"];
        [atime setValue:[dataFormater dateFromString:self.daochangTime.text] forKey:@"time"];
        
        [[AppDelegate App]saveContext];
        atime=[Times newDataObjectWithEntityName:@"Times"];
        [atime setValue:@"tuocheTime" forKey:@"name"];
        [atime setValue:caseID forKey:@"parent_id"];
        [atime setValue:[dataFormater dateFromString:self.tuocheTime.text] forKey:@"time"];
        
        [[AppDelegate App]saveContext];
        atime=[Times newDataObjectWithEntityName:@"Times"];
        [atime setValue:@"jiaojingTime" forKey:@"name"];
        [atime setValue:caseID forKey:@"parent_id"];
        [atime setValue:[dataFormater dateFromString:self.jiaojingTime.text] forKey:@"time"];
        ...
        [atime setValue:caseID forKey:@"parent_id"];
        [atime setValue:[dataFormater dateFromString:self.overTime.text] forKey:@"time"];
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    if (textField.tag==12 || textField.tag==13 || textField.tag==10 || textField.tag==555 || textField.tag==556 || textField.tag==557) {
        return NO;
//    }else if (textField.tag == 1007 || textField.tag == 1008 || textField.tag == 1009 ){
//        switch (textField.tag) {
//            case 1007:
//                [self pickerPresentForIndex:0 fromRect:[textField frame]];
//                break;
//            case 1008:
//                [self pickerPresentForIndex:2 fromRect:[textField frame]];
//                break;
//            case 1009:
//                [self pickerPresentForIndex:1 fromRect:[textField frame]];
//                break;
//            default:
//                break;
//        }
//        return NO;
//    }else if (textField.tag == 61 || textField.tag == 62 || textField.tag == 63 || textField.tag == 64 || textField.tag == 65){
//        [self theTimes:textField];
//        return NO;
    }
    else {
        return YES;
...
-(void)loadDataForCase:(NSString *)caseID{
    ...
    if (caseInfo) {
        ...
        //默认为0
        _textreason.text     = caseInfo.case_reason;            //安全距离不足   事故原因
        _textCaseStyle.text  = caseInfo.case_style;             //一般事故      事故性质
        _textCaseType.text   = caseInfo.case_type;              //碰撞护栏及交通设施     事故类型
    ...
    if(times.count>0){
        for (Times *atime in times) {
            NSString* name =[atime valueForKey:@"name"];
            [[self valueForKey:name] setValue:[dataFormater stringFromDate:atime.time] forKey:@"text"];
        }
    }
...
- (IBAction)theTimes:(UITextField *)sender {
    ...
    [self.pickerPopover presentPopoverFromRect:[sender frame] inView:self.view permittedArrowDirections:UIPopoverArrowDirectionLeft animated:YES];
    datePicker.dateselectPopover = self.pickerPopover;
    datePicker.setDataBolck=^(NSDate*date){
        NSDateFormatter *dateFormator=[[NSDateFormatter alloc]init];
        dateFormator.timeZone        = [NSTimeZone systemTimeZone];
        dateFormator.dateFormat=@"yyyy-MM-dd HH:mm";
        NSString *timestring=[dateFormator stringFromDate:date];
        //NSString *timestring         = date;
        switch (sender.tag) {
            case 61:
                self.jiebaoTime.text = timestring;
                break;
            case 62:
                self.daochangTime.text = timestring;
                break;
            case 63:
                self.tuocheTime.text = timestring;
                break;
            case 64:
                self.jiaojingTime.text = timestring;
                break;
            case 65:
                self.overTime.text = timestring;
                break;
            default:
                break;
        }
