...
@class Times;
@interface CaseViewController : UIViewController<UITextFieldDelegate,CaseIDHandler,DatetimePickerHandler,UIImagePickerControllerDelegate,UINavigationControllerDelegate,RoadSegmentPickerDelegate,ReloadPaintDelegate,AipOcrDelegate,UIPopoverPresentationControllerDelegate,UIPopoverPresentationControllerDelegate,UIScrollViewDelegate>
...
- (IBAction)btnImageFromCamera:(id)sender;      //调用相机
- (IBAction)btnImageFromLibrary:(id)sender;
- (IBAction)btnClickToEditor:(id)sender;
- (IBAction)btnNewCase:(id)sender;              //新增案件按钮
- (IBAction)selectWeather:(id)sender;
- (IBAction)selectRoadSegmet:(UITextField *)sender;      //选择路段
- (IBAction)selectRoadSide:(UITextField *)sender;           //往   南，西
- (IBAction)selectRoadPlace:(UITextField *)sender;          //左侧 ，右侧
...
- (IBAction)btnSaveCaseInfo:(id)sender;         //保存案件信息按钮
- (IBAction)btnPreviousCase:(id)sender;         //已立历史案件
@property (weak, nonatomic) IBOutlet UIBarButtonItem *Previouscaserightbarbutton;//已立案件
