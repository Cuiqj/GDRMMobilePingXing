...
#define SCROLLVIEW_HEIGHT 580.0
@interface BaoxianCaseViewController(){
...
//@property (nonatomic,strong) XLPhotoBrowser *xlPhotoBroser;
@property (nonatomic,strong) PictureSacnViewController *PictureSacnViewControllerVC;
...
@implementation BaoxianCaseViewController
...
//-(XLPhotoBrowser*)xlPhotoBroser{
//    if(!_xlPhotoBroser){
//        _xlPhotoBroser=[XLPhotoBrowser showPhotoBrowserWithImages:self.images currentImageIndex:0];
//    }
//    return _xlPhotoBroser;
//}
-(PictureSacnViewController*)PictureSacnViewControllerVC{
    if(_PictureSacnViewControllerVC==nil){
        _PictureSacnViewControllerVC=[PictureSacnViewController new];
        _PictureSacnViewControllerVC.picArray=[NSMutableArray arrayWithArray:self.images];
        _PictureSacnViewControllerVC.currenpage = 2;
    }
    return _PictureSacnViewControllerVC;
}
-(NSArray*)images{
    if(_images==nil){
...
-(NSArray *)caseDescArray{
    if (_caseDescArray==nil) {
        ...
        _caseDescArray=[NSArray newBaoxianCaseDescArray];
    }
...
-(NSArray*)docTmplArray{
    if (_docTmplArray == nil) {
        ...
        _docTmplArray = DOC_TEMPLATES_INSURANCE_NAME_ARRAY;
    }
...
//    [self addleftofRightBarButton];
    ...
    proveDate        = nil;
    touchedTag=-1;
    self.textRoadSegment.font = [UIFont systemFontOfSize:12];
...
//    self.deformInfoVC.deformTableView.frame = CGRectMake(0.0, 0.0, SCROLLVIEW_WIDTH,SCROLLVIEW_HEIGHT);
//    [self.infoView addSubview:self.deformInfoVC.deformTableView];
...
//    加载赔偿清单
    self.deformInfoVC.deformTableView.frame = CGRectMake(0.0, 0.0, SCROLLVIEW_WIDTH,SCROLLVIEW_HEIGHT);
    self.deformInfoVC.citizenPickerView.frame = CGRectMake(0, 0, 0, 0);
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    if (self.roadInspectVC && [[self.navigationController visibleViewController] isEqual:self.roadInspectVC]) {
        if (![self.inspectionID isEmpty]) {
            [self.roadInspectVC createRecodeByBaoxianCase:self.caseID];
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField {
    ...
    //    }else
    if (textField ==self.textStationStartKM) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    //    }
    //    return number;
    return  self.docTmplArray.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *CellIdentifier = @"CaseDocumentListCell";
    cell                            = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    cell.textLabel.text             = self.docTmplArray[indexPath.row];
    NSString *name                  = self.docTmplArray[indexPath.row];
    int  a                          = [self.docListArray indexOfObject:name];
...
- (IBAction)selectCaseType:(id)sender{
    ...
    if([self.caseInfoPickerpopover isPopoverVisible]){
        ...
        [self.caseInfoPickerpopover presentPopoverFromRect:[(UITextField*)sender frame] inView:self.view permittedArrowDirections:UIPopoverArrowDirectionLeft animated:YES];
...
-(void) setCaseType:(NSString *)textCaseType{
    if(touchedTag==1111){
        self.textcase_type.text         = textCaseType;
    }else if(touchedTag==2222){
        self.textinsurance_company.text = textCaseType;
    }
}
//各信息页面切换
    ...
    switch (sender.selectedSegmentIndex) {
        ...
        case 2:{
            ...
            [UIView transitionWithView:self.infoView duration:0.3
                            ...
                            animations:^{
                                ...
                                if (self.caseInfo.isdowndata.integerValue == 1){
                                    [self.uiButtonEdit setHidden:YES];
                                }
                            }
                            completion:^(BOOL finished){
        ...
        case 1:{
            ...
            /*
             ...
             [self.uiButtonPickFromLibrary setHidden:NO];
             [self.uiButtonPickFromLibrary setAlpha:1.0];
             //self.xlPhotoBroser=[XLPhotoBrowser showPhotoBrowserWithImages:self.images currentImageIndex:0];
             [self.infoView addSubview:self.PictureSacnViewControllerVC.view];
             }
             completion:^(BOOL finished){
             [self.uiButtonEdit setHidden:YES];
             UILongPressGestureRecognizer *longPressGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(showDeleteMenu:)];
...
- (IBAction)btnNewCase:(id)sender;{
    ...
    _PictureSacnViewControllerVC.picArray=[NSMutableArray array];
...
- (IBAction)btnPreviousCase:(id)sender {
    if ([self.caseListpopover isPopoverVisible]) {
        ...
        caseListVC.caseType  = CaseTypeIDBaoxian;
...
- (void)loadCaseInfoForCase:(NSString *)caseID{
    ...
    if (self.caseInfo) {
        ...
        self.textWeatheer.text  = self.caseInfo.weater;           //天气
        self.textRoadSegment.text = [RoadSegment roadNameAndroadFromSegment:self.caseInfo.roadsegment_id];     //地点
        self.roadSegmentID      = self.caseInfo.roadsegment_id;
        self.textPlace.text     = self.caseInfo.place;            //右侧
        self.textSide.text      = self.caseInfo.side;               //往东

        self.textcase_type.text         = self.caseInfo.case_type;
        self.textcase_type.text         = self.caseInfo.case_type;
        self.textinsurance_no.text      = self.caseInfo.insurance_no;
        self.textinsurance_company.text = self.caseInfo.insurance_company;
...
- (void)saveCaseInfoForCase:(NSString *)caseID{
    ...
    self.caseInfo.case_type_id=@"20";

    self.caseInfo.case_type         = self.textcase_type.text;
    self.caseInfo.insurance_no      = self.textinsurance_no.text;
    self.caseInfo.insurance_company = self.textinsurance_company.text;
    ...
    [self saveCaseCitizenInfoforCase:caseID];
//    [self saveCitizenInfoForCase:caseID andAutoNumber:@"车号"];  案件信息里是车号
    switch (self.segInfoPage.selectedSegmentIndex) {
            //        case 0:
...
- (void)saveCaseCitizenInfoforCase:(NSString *)caseID{
    NSString * Casetype = self.textcase_type.text;
     self.citizenInfo = [Citizen citizenByCaseID:self.caseID];
    if(!self.citizenInfo){
        self.citizenInfo = [Citizen newDataObjectWithEntityName:@"Citizen"];
        self.citizenInfo.proveinfo_id = self.caseID;
        self.citizenInfo.nexus = @"当事人";
    }
    if ([Casetype containsString:@"偷盗"]){
        _citizenInfo.org_name= @"被盗";
        _citizenInfo.party = @"无当事人";
    }else if ([Casetype containsString:@"自然灾害"]){
        _citizenInfo.org_name= @"自然灾害";
        _citizenInfo.party = @"无当事人";
    }else if ([Casetype containsString:@"逃逸"]) {
        _citizenInfo.org_name= @"逃逸事故";
        _citizenInfo.party = @"事主逃逸";
    }else{
        _citizenInfo.org_name= @"逃逸事故";
        _citizenInfo.party = @"事主逃逸";
    }
...
- (void)saveCaseProveInfoForCase:(NSString *)caseID{
    ...
    if([self.caseInfo.station_start integerValue] ==0 && [self.caseInfo.station_end integerValue] ==0){
        stationString = self.caseInfo.place;
    ...
    //现在的勘验场所描述
    caseProveInfo.remark = [NSString stringWithFormat:@"%@%@%@",[RoadSegment roadNameFromSegment:self.caseInfo.roadsegment_id],self.textSide.text,stationString];
...
- (void)loadCasePhotoForCase:(NSString *)caseID{
    ...
    self.imageIndex    = 0;
    for (NSString* name in self.photoArray) {
        NSString* caseimgurlstring=[self.photoPath stringByAppendingPathComponent:name];
        UIImage *img=[UIImage imageNamed:caseimgurlstring];
        // [self.images addObject:img];
        NSMutableDictionary *dic=[NSMutableDictionary dictionary];
        [dic setObject:@"1" forKey:@"islocal"];
        [dic setObject:name forKey:@"text"];
        [dic setObject:img forKey:@"picString"];
    ...
    if (![self.caseID isEmpty]) {
        ...
        dispatch_async(myqueue, ^(void){
            ...
            if ([photoData writeToFile:filePath atomically:YES]) {
                ...
                NSMutableDictionary *dic=[NSMutableDictionary dictionary];
                [dic setObject:@"1" forKey:@"islocal"];
                [dic setObject:photoName forKey:@"text"];
                [dic setObject:[UIImage imageNamed:filePath] forKey:@"picString"];
                [self.images addObject:dic];
                self.PictureSacnViewControllerVC.picArray=[NSMutableArray arrayWithArray:self.images];
...
- (void)sideclickAction{
    [self roadSegmentPickerPresentPickerState:kRoadSide fromRect:self.textSide.frame];
}
- (void)placeclickAction{
    [self roadSegmentPickerPresentPickerState:kRoadPlace fromRect:self.textPlace.frame];
}
- (void)sideLongPress{
    [self.textSide becomeFirstResponder];
}
- (void)placeLongPress{
    [self.textPlace becomeFirstResponder];
}
//添加手势
- (void)addGestureRecognizer{
    ...
    CGRect sideframe = self.textSide.frame;
    UIView * sideview = [[UIView alloc] initWithFrame:sideframe];
    sideview.tag = 1111;
    [self.view addSubview:sideview];
    //    sideview.backgroundColor = [UIColor blackColor];
    [sideview addGestureRecognizer:sideclick];
    [sideview addGestureRecognizer:sidelongPressGesture];
    UITapGestureRecognizer *placeclick = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(placeclickAction)];
    UILongPressGestureRecognizer * placelongPressGesture =[[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(placeLongPress)];
    placelongPressGesture.minimumPressDuration=0.5f;
    CGRect placeframe = self.textPlace.frame;
    UIView * placeview = [[UIView alloc] initWithFrame:placeframe];
    [self.view addSubview:placeview];
    placeview.tag = 1112;
    [placeview addGestureRecognizer:placeclick];
    [placeview addGestureRecognizer:placelongPressGesture];
}

#pragma mark - 添加已上传案件查看按钮
- (void)addleftofRightBarButton{
    ...
    self.navigationItem.rightBarButtonItems = [NSArray arrayWithObjects: self.Previouscaserightbarbutton,btn0,btn1,nil];
}
-(void)downloadDataShow:(UIBarButtonItem *)barbutton{
    if ([self.caseListpopover isPopoverVisible]) {
        [self.caseListpopover dismissPopoverAnimated:YES];
    } else {
        //        CaseinfoDownListViewController *caseListVC=[[CaseinfoDownListViewController alloc] init];
        CaseinfoDownListViewController * caseListVC=[self.storyboard instantiateViewControllerWithIdentifier:@"CaseinfoDownListView"];
        caseListVC.caseType  = CaseTypeIDBaoxian;
        self.caseListpopover=[[UIPopoverController alloc] initWithContentViewController:caseListVC];
        caseListVC.delegate  = self;
        caseListVC.myPopover = self.caseListpopover;
        [self.caseListpopover presentPopoverFromBarButtonItem:barbutton permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
    }
}
@end
