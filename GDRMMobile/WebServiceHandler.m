...
-(void)executeWebService:(NSString *)serviceName
    ...
    [NSURLConnection sendAsynchronousRequest:theRequest queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error){
        [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:NO];
        //        dispatch_queue_t paraseDispatch=dispatch_queue_create("myThreadQueue1", DISPATCH_QUEUE_SERIAL);
//        dispatch_async(paraseDispatch, ^{
//        
//            if (data.length > 0 && error == nil) {
//                NSString *theXML = [[NSString alloc] initWithBytes: [data bytes] length:[data length] encoding:NSUTF8StringEncoding];
...
#pragma mark  下载案件或巡查数据
//downloadDataSet:downstr withtypeID:typeID addtable:@"CaseInfo"];
- (void)downloadDataSet:(NSString *)strSQL withtypeID:(NSString *)typeID addtable:(NSString *)table{
    NSString *soapMessage = [[NSString alloc] initWithFormat:@" <DownloadDataSet xmlns=\"http://tempuri.org/IrmsData/MobileData\"> \n"
                             "  <key>%@</key> \n"
                             "  <strSQL>%@</strSQL> \n"
                             " </DownloadDataSet> \n",PASSWORD,strSQL];
    [self executeWebService:@"DownloadDataSet" serviceParm:soapMessage addtable:table withtypeID:typeID];
}
-(void)executeWebService:(NSString *)serviceName
             serviceParm:(NSString *)parms addtable:(NSString *)table withtypeID:(NSString *)typeID {
    //web service request
    NSString *soapMessage=[NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"utf-8\"?>"
                           "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" \n"
                           "xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\"  \n"
    ...
    [NSURLConnection sendAsynchronousRequest:theRequest queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error){
        ...
        if (data.length > 0 && error == nil) {
            NSString *theXML = [[NSString alloc] initWithBytes: [data bytes] length:[data length] encoding:NSUTF8StringEncoding];
            [self.delegate getWebServiceReturnString:theXML forWebService:serviceName adddataModel:table withtypeID:typeID];
        } else if (error != nil && error.code == NSURLErrorTimedOut) {
            [self.delegate requestTimeOut];
        } else if (error != nil) {
            [self.delegate requestUnkownError];
