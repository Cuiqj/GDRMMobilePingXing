...
- (void)loadInspectionInfo:(BOOL)isdowndata;
- (void)saveRemark;
@end
...
- (void)reloadRecordData:(BOOL)isdowndata{
    if (self.state == kRecord) {
        self.data=[[InspectionRecord recordsForInspection:self.inspectionID andisdowndata:isdowndata] mutableCopy];
...
- (void)loadInspectionInfo:(BOOL)isdowndata{
    NSArray *temp;
    if (isdowndata) {
        temp = [Inspection inspectionDownDataForID:self.inspectionID];
    }else{
        temp =[Inspection inspectionForID:self.inspectionID];
    }
    if (temp.count>0) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillShow:) name:UIKeyboardWillShowNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillHide:) name:UIKeyboardWillHideNotification object:nil];
    [self reloadRecordData:isdowndata];
}
...
- (void)setCurrentInspection:(NSString *)inspectionID anddowndata:(BOOL)isdowndata{
    ...
    [self loadInspectionInfo:isdowndata];
}
...
-(void)downloadDataShow:(UIBarButtonItem *)barbutton{
    if ([self.pickerPopover isPopoverVisible]) {
        ...
        InspectionDownDataListViewController *acPicker=[self.storyboard instantiateViewControllerWithIdentifier:@"InspectionDownDataList"];
        acPicker.delegate = self;
        self.pickerPopover=[[UIPopoverController alloc] initWithContentViewController:acPicker];
        [self.pickerPopover setPopoverContentSize:CGSizeMake(270, 352)];
        [self.pickerPopover presentPopoverFromBarButtonItem:barbutton permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
        acPicker.popover = self.pickerPopover;
    }
}
