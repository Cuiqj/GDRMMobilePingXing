...
+ (id)newDataObjectWithEntityName:(NSString *)entityName{
    ...
    if ([obj respondsToSelector:@selector(isdowndata)]) {
        [obj setValue:@(NO) forKey:@"isdowndata"];
    }
...
+ (NSString *)complexTypeString{
    ...
    for (NSString *attriName in [attributes allKeys]) {
        if (![attriName isEqualToString:@"isuploaded"] && ![attriName isEqualToString:@"isdowndata"]) {
...
-(NSString *)dataXMLStringForCaseMap{
    ...
    "<data>%@</data>"
    "<remark>%@</remark>";
    CasePhoto * map= (CasePhoto *)self;
    NSArray *pathArray=NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentPath=[pathArray objectAtIndex:0];
    NSString *mapPath=[NSString stringWithFormat:@"CaseMap/%@",map.myid];
    ...
    NSData * data = UIImageJPEGRepresentation(image,1.0);
    NSString * stringImage = [NSString base64forData:data];
    casePhotoStr = [NSString stringWithFormat:casePhotoStr,map.myid?map.myid:@"",map.proveinfo_id?map.proveinfo_id:map.proveinfo_id,map.photo_name?map.photo_name:@"",stringImage?stringImage:@"",map.remark?map.remark:@""];
    return casePhotoStr;
}


#pragma mark - 下载数据的选择     获取下载数据
+ (NSArray *)selectedisuploadedArrayOfObjectwithtype:(NSString *)typeID{
    NSManagedObjectContext *context=[[AppDelegate App] managedObjectContext];
    NSEntityDescription *entity=[NSEntityDescription entityForName:NSStringFromClass([self class]) inManagedObjectContext:context];
    NSFetchRequest *fetchRequest=[[NSFetchRequest alloc] init];
    [fetchRequest setEntity:entity];
    //    if ([NSStringFromClass([self class]) isEqualToString:@"CasePhoto"]) {
    //        [fetchRequest setPredicate:[NSPredicate  predicateWithFormat:@"isuploaded.boolValue == YES && (proveinfo_id != nil||project_id!=nil)"]];
    //        //[fetchRequest setPredicate:[NSPredicate  predicateWithFormat:@"isuploaded.boolValue == NO"  ]];
    //    }else if ([NSStringFromClass([self class]) isEqualToString:@"CarCheckRecords"]) {
    //        [fetchRequest setPredicate:[NSPredicate  predicateWithFormat:@"isuploaded.boolValue == YES &&  parent_id != nil "]];
    //        //[fetchRequest setPredicate:[NSPredicate  predicateWithFormat:@"isuploaded.boolValue == NO"  ]];
    //    }else if ([NSStringFromClass([self class]) isEqualToString:@"RoadAsset_Check_detail"]) {
    //        [fetchRequest setPredicate:[NSPredicate  predicateWithFormat:@"isuploaded.boolValue == YES &&  status ==1 "]];
    //        //[fetchRequest setPredicate:[NSPredicate  predicateWithFormat:@"isuploaded.boolValue == NO"  ]];
    //    }else if ([NSStringFromClass([self class]) isEqualToString:@"BridgeCheckDetail"]) {
    //        [fetchRequest setPredicate:[NSPredicate  predicateWithFormat:@"isuploaded.boolValue == YES &&  status ==1 "]];
    //        //[fetchRequest setPredicate:[NSPredicate  predicateWithFormat:@"isuploaded.boolValue == NO"  ]];
    //    }else{
    if (typeID != nil) {
        [fetchRequest setPredicate:[NSPredicate  predicateWithFormat:@"isdowndata.boolValue == YES && case_type_id == %@",typeID]];
    }else{
        [fetchRequest setPredicate:[NSPredicate  predicateWithFormat:@"isdowndata.boolValue == YES "]];
    }
    return [context executeFetchRequest:fetchRequest error:nil];
}
