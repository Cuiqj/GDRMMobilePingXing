...
-(void)addDeformationForRoadAsset:(RoadAssetPrice *)aRoadAsset andQuantity:(double)quantity withRemark:(NSString *)aRemark withDegree:(NSString *)aDegree{
    ...
    if (![self.caseID isEmpty]) {
        ...
        if (self.citizenList.count>0||[caseinfo.case_type_id isEqualToString:@"20"]) {
            ...
            if ([caseinfo.case_type_id isEqualToString:@"20"]) {
                rowTitle=@"保险案件";
            }
...
-(void)NotcaseshowAlertView{
    UIAlertView *alertView=[[UIAlertView alloc] initWithTitle:@"错误" message:@"无法记录损坏路产，请检查案件信息是否填写完整或保存！" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
    [alertView show];
}
...
-(void)reloadInfo{
    ...
    if (self.citizenList.count>0 ||[caseinfo.case_type_id isEqualToString:@"20"]) {
        ...
        if(self.citizenList.count<=0){
            ...
            citizenName=[self.citizenList objectAtIndex:0];
        }
...
-(void)reloadDeformTableViewWithCitizenName:(NSString *)citizenName{
    ...
    if (self.citizenList.count>0) {
        ...
        self.deformList=[[CaseDeformation deformationsForCase:self.caseID] mutableCopy];
    }
    [self.deformTableView reloadData];
}
